...
public class PersonServiceImpl extends SuperImpl implements PersonService {
    ...
    public PersonServiceImpl(int port) throws RemoteException {
        super(port);
        picture_dir = System.getProperty("user.dir") + "/" + "pic";
        file_dir = System.getProperty("user.dir") + "/" + "file";
    }
    ...
    public void changePersonChange(Transaction tx, Session session, HashSet<BasePersonChange> set, ChangeScheme changeScheme, RyChgLog model_log) throws Exception {
        ...
        for (String entity_name : entitys) {
            String hql = " select a01_key,max(a_id) from " + entity_name + " where a01_key in (" + a01Key + ") group by a01_key";
        ...
        ci_sql += "select " + key + ",a01.a01_key,'" + ip + "','" + mac + "','" + user + "'," + now + ",a01.a0190,a01.a0101,d.content,ac.chgfieldname,'" + changeScheme.getChangeScheme_name() + "'";
        ci_sql += ",ac.b_dept_code,ac.a_dept_code,ac.beforestate,ac.afterstate,'" + changeScheme.getChangeScheme_key() + "' from A01chg ac,BasePersonChange bpc,A01 a01,DeptCode d where ac.basepersonchange_key=bpc.basepersonchange_key and bpc.a01_key=a01.a01_key and a01.deptCode_key=d.deptCode_key"
        ...
        for (ChangeItem ci : changeScheme.getChangeItems()) {
            s_select_bpc = s_select_bpc + "," + changeTable + "." + PersonUtil.getDbFieldName(ci.getFieldName(), true);
            s_select_a01 = s_select_a01 + ",A01." + PersonUtil.getDbFieldName(ci.getFieldName(), false);
        }
        for (ChangeField cf : changeScheme.getChangeFields()) {
            if (cf.getAppendix_name() == null || !"A01".equals(cf.getAppendix_name())) {
                continue;
            }
            ...
            if (cf.isFrom_import()) {
                ...
                if ("BasePersonChange".equals(importTable) && (importField.startsWith("new_") || importField.startsWith("old_"))) {
                    importTable = changeTable;
                }
            }
            s_select_bpc = s_select_bpc + "," + importTable + "." + importField;
            s_select_a01 = s_select_a01 + ",A01." + fieldName;
        }
        a01_sql += "(" + s_select_a01.substring(1) + ")=( select " + s_select_bpc.substring(1) + " from " + changeTable + " ,BasePersonChange  where A01.a01_key=BasePersonChange.a01_key and " + changeTable + ".basepersonchange_key=BasePersonChange.basepersonchange_key and BasePersonChange.basepersonchange_key in (" + bpcKey + ")) ";
        a01_sql += " where a01_key in(" + a01Key + ");";
        ex_sql += a01_sql;
        String zp_sql = "update Zp_resume set z_state='入职结束' where a01_key in(" + a01Key + ");";
        ex_sql += zp_sql;
        if (!chgHTKey.equals("") && change_ht_flag) {
            ex_sql += "update HT01 ht01 set deptCode_key=(select deptCode_key from A01 where ht01.a01_key=A01.a01_key) where ht01.a01_key in (" + chgHTKey + ");";
        }
...
//                if (appendixMethod.contains("更新") && "更新".equals(ce.getC_type())) {
                ...
                if (appendixMethod.contains("追加") && !"更新".equals(ce.getC_type())) {
                    BasePersonAppendix bpa = tmp_table.get(entity);
                    if (bpa == null) {
                        try {
        ...
        if (b_include_type) {
            ...
            for (ChangeField cf : changeScheme.getChangeFields()) {
                ...
                s_field += "," + importTable + "." + importField;
            ...
            if (!s_select.equals("")) {
                ...
                s_field = s_field.substring(1);
                sql += "update " + newPersonClass + " set (" + s_select + ")" + "=(select " + s_field + " from " + newPersonClass + " a01," + s_from + " where " + s_where
                        + " and BasePersonChange.basepersonchange_key in(" + bpcKey + ")) where a01_key in(" + a01Key + ").";
            }
        }
        if ("借调".equals(changeScheme.getScheme_type())) {
            ...
            if (mod == 2) {
                for (BasePersonChange bpc : set) {
                    ...
                    sql += "insert into A01jdLog(a01jdlog_key,a01_key,old_deptcode_key,new_deptcode_key,c_date,c_user,old_dept_code,old_dept_content,new_dept_code,new_dept_content,c_type)values(sys_guid()," + a01_key + ",'" + old_dept.getDeptCode_key() + "','"
            ...
            for (BasePersonChange bpc : set) {
                ...
                sql += "insert into A01tempchangelog(a01tempchangelog_key,a01_key,old_deptcode_key,new_deptcode_key,c_date,c_user,old_dept_code,old_dept_content,new_dept_code,new_dept_content,c_type)values(sys_guid()," + a01_key + ",'" + old_dept.getDeptCode_key() + "','"
    ...
    public ValidateSQLResult saveChangeScheme(ChangeScheme changeScheme, QueryScheme queryScheme, boolean tableUpdate, String roleKey) throws RemoteException {
        ...
        try {
            ...
            sql = "insert into TABNAME(entity_key,entityName,entitycaption,canmodify,order_no,success_build,limit_flag,init_flag)"
                    + "select  '" + entityName + "','" + entityName + "','" + schemeName + "','其他业务'," + order + ",0,0,0 from changescheme where changeScheme_key='" + schemeKey + "'"
                    + " and not exists(select 1 from TabName where entityName='" + entityName + "') and rownum<2;";
            sql += "update TabName t set (entityClass_key,entitycaption)=(select ec.entityClass_key,'" + schemeName + "' from EntityClass ec where ec.entityType_code='RSDD') where t.entityName='" + entityName + "';";
            ImplUtil.exSQLs(tx, session, sql, ";");
...
//            List no_list = s.createSQLQuery("select changeScheme_no from changeScheme where changeScheme_key<>'EmpScheme_Del'").list();
            ...
            ImplUtil.exSQLs(tx, s, validateSQLResult, ex_sqls, ";");
        } catch (Exception e) {
            ImplUtil.rollBack(tx);
            validateSQLResult.setResult(1);
            validateSQLResult.setMsg(ImplUtil.getSQLExceptionMsg(e));
    ...
    public ValidateSQLResult addPersonForNoCheck(List<String> list, RyChgLog rc) throws RemoteException {
        ...
        String ci_sql = "insert into RyChgLog(rychglog_key,a01_key,chg_ip,chg_mac,chg_user,chg_date,a0190,a0101,dept_name,chg_field,chg_type,old_dept_code,new_dept_code,beforestate,afterstate,changeScheme_key) ";
        ci_sql += "select sys_guid(),rcl.a01_key,'" + rc.getChg_ip() + "','" + rc.getChg_mac() + "','" + rc.getChg_user() + "'," + DateUtil.toStringForQuery(new Date(), "yyyy-MM-dd hh:mm:ss", "oracle") + ",rcl.a0190,rcl.a0101,rcl.dept_name,'a0193','新增'";
    ...
    public ValidateSQLResult changePersonNo(List<String[]> nos, String no_field) throws RemoteException {
        ...
        String db_type = HibernateUtil.getDb_type();
        String create_sql = "create table " + tableName + "(a01_key varchar2(50) primary key,p_no int)";
        try {
            ...
            session.flush();
            create_sql = "update A01 set " + no_field + "=(select b.p_no from " + tableName + " b where A01.a01_key=b.a01_key) where exists(select 1 from " + tableName + " b where b.a01_key=A01.a01_key)";
            session.createSQLQuery(create_sql).executeUpdate();
    ...
    public ValidateSQLResult deleteEmpDocu(List<String> edKeys) throws RemoteException {
        ...
        try {
            String db_type = HibernateUtil.getDb_type();
            String create_sql = "create table " + tableName + "(EmpDocu_key varchar2(50))";
            String ex_sql = DbUtil.isTableExistSQL(tableName, db_type);
            int i = Integer.valueOf(session.createSQLQuery(ex_sql).uniqueResult().toString());
            if (i != 0) {
                session.createSQLQuery("drop table " + tableName).executeUpdate();
            }
            session.createSQLQuery(create_sql).executeUpdate();
    ...
    public ValidateSQLResult saveAnnexCheck(List entityList) throws RemoteException {
        ...
        try {
            ...
            for (Object obj : entityList) {
                ...
                sql_check_flag += " select '" + UtilTool.getUID() + "',null,'',1,0,1,'审核状态','自定义已选项','" + field + "',0,'String',20,'',0,0,1,'20',1,1,1,entity_key,'SHZT','左对齐',0,0,0,'null','null',0,null,0,0,0,0,0,0 from tabname where entityname = '"
                        + entity + "' and not exists(select 1 from system where field_name ='" + field + "' and entity_key in(select entity_key from tabname where entityname = '" + entity + "')) and rownum<2";
                sql_person_code += " select '" + UtilTool.getUID() + "',null,'',1,0,1,'新增人','自定义已选项','person_code',0,'String',20,'',0,1,1,'20',0,0,0,entity_key,'XZR','左对齐',0,0,0,'null','null',0,null,0,0,0,0,0,0 from tabname where entityname = '"
                        + entity + "' and not exists(select 1 from system where field_name ='person_code' and entity_key in(select entity_key from tabname where entityname = '" + entity + "')) and rownum<2";
                String sql2 = " declare "
                        + " cnt number; "
                        + " begin "
                        + " SELECT count(1) into cnt from user_tab_columns where column_name = '" + field.toUpperCase() + "' and table_name = '" + entity + "'; "
                        + " if cnt = 0 then "
                        + " execute immediate 'alter table " + entity + " add " + field + " varchar2(255)';"
                        + " end if;"
                        + "commit;"
                        + "end;";
                ...
                sql2 = " declare "
                        + " cnt number; "
                        + " begin "
                        + " SELECT count(1) into cnt from user_tab_columns where column_name = 'PERSON_CODE' and table_name = '" + entity + "'; "
                        + " if cnt = 0 then "
                        + " execute immediate 'alter table " + entity + " add person_code varchar2(255)';"
                        + " end if; "
                        + " commit;"
                        + " end;";
                s.createSQLQuery(sql2).executeUpdate();
    ...
    public ValidateSQLResult delAppendix(String appendixTable, List<String[]> aKeys) throws RemoteException {
        ...
        ValidateSQLResult validateSQLResult = new ValidateSQLResult();
        String db_type = HibernateUtil.getDb_type();
        String create_sql = "create table " + tableName + "(t_key varchar2(100),a01_key varchar2(50),a_id varchar2(50),last_flag varchar2(10))";
        try {
            ...
            if (!data.isEmpty()) {
                ...
                ex_sql += "update " + appendixTable + " a set (a_id,last_flag)=(select b.a_id,b.last_flag from " + tableName + " b where a.basePersonAppendix_key=b.t_key) where exists(select 1 from " + tableName + " b where a.basePersonAppendix_key=b.t_key);";
    ...
    public List getDocumentPerson(List<String> depts, Set<String> a0190s, String person_key) throws RemoteException {
        ...
        Transaction tx = s.beginTransaction();
        String db_type = HibernateUtil.getDb_type();
        String create_sql = "create table " + tableName + "(a0190 varchar2(255),a_id int,a01_key varchar2(255),dept_code varchar2(255))";
        String deptStr = "";
        ...
        try {
            ...
            s.createSQLQuery(create_sql).executeUpdate();
            s.flush();
            String sql = DbUtil.getInstrForMID("insert into " + tableName + "(a0190,a_id)", data, "", ";");
            ImplUtil.exSQLs(tx, s, sql, ";");
                ...
                if (fieldName.equals("deptCode")) {
                    ...
                    if (fieldName.equals("a0191")) {
                        ...
                        for (Object data : list) {
                            ...
                            if (objs[1].toString().equals(obj.toString())) {
    ...
    public ValidateSQLResult addAppendixs(List apppendixs) throws RemoteException {
        ...
        String tableName = CountUtil.getCommTempTable();
        String db_type = HibernateUtil.getDb_type();
        String create_sql = "create table " + tableName + "(a01_key varchar2(50) primary key)";
        Session s = HibernateUtil.currentSession();
        Transaction tx = s.beginTransaction();
        try {
            DbUtil.initTempTable(s, tableName, db_type);
            ...
            for (String entity : entitys) {
                String ex_sql = "delete from " + tableName + ";";
                ex_sql += DbUtil.getInstrForMID("insert into " + tableName + "(a01_key)", appendixA01Keys.get(entity), " from dual", ";");
